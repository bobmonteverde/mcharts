...
mc.models.legend = function legend(model) {
  ...
  chart.calc = function(instance, data) {
    ...
    let series   = data; //TODO: temp not using model.series_ due to it filtering disabled

    instance.legendElements = series.map((s, j) => ({
      series: s,
    ...
    let column = 0;
    let row = 0;
    let left = model.padding.left;
    let top = model.padding.top;

    instance.legendElements.forEach(elem => {
      elem.column = column;
      elem.row    = row;
      elem.left   = left;
      elem.top    = top;
    ...
    if (model.horizontalAlign === 'center')
      instance.leftOffset = parseInt((instance.width - instance.innerWidth) / 2, 10);
    else if (model.horizontalAlign === 'right')
      instance.leftOffset = instance.width - instance.innerWidth;
    else
      instance.leftOffset = 0;

    if (model.verticalAlign === 'center')
      instance.topOffset = parseInt((instance.height - instance.innerHeight) / 2, 10);
    else if (model.verticalAlign === 'bottom')
  ...
  chart.build = function(instance, data) {
    ...
    instance.gEnter.append('g').attr('class', 'mc-legendElements')
        .attr('transform', `translate(${instance.leftOffset},${instance.topOffset})`); // setting to prevent weird slide animation on first load //TODO: consider no transitioning this
    ...
    instance.g.select('.mc-legendElements').transition()
        .attr('transform', `translate(${instance.leftOffset},${instance.topOffset})`);


    let elements = instance.g.select('.mc-legendElements').selectAll('.mc-legendElement')
        .data(instance.legendElements);

    let elementsEnter = elements.enter().append('g')
        ...
        .attr('class', (d, i) => `mc-legendElement mc-legendElement-${i} mc-group-${i}`)
    ...
    elements
        .classed('mc-disabled', d => d.series.disabled);
    ...
    elements.select('path')
        .attr('transform', d => `translate(7,${d.height / 2})`)
        .attr('d', function(d) {
  ...
  chart.legendOrientation = function(_) {
    ...
    model.orientation = (_ !== 'horizontal' && _ !== 'vertical') ? 'horizontal' : _;
    return chart;
  };

  chart.legendHorizontalAlign = function(_) {
    if (!arguments.length) return model.horizontalAlign;
    model.horizontalAlign = (['left', 'center', 'right'].indexOf(_) === -1) ? 'center' : _;
    return chart;
  };

  chart.legendVerticalAlign = function(_) {
    if (!arguments.length) return model.verticalAlign;
    model.verticalAlign = (['top', 'center', 'bottom'].indexOf(_) === -1) ? 'center' : _;
    return chart;
  };
