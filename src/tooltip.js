...
(function(window, document, mc) {
  ...
  var mctooltip   = {}
    , removeDelay = 500 // ms to wait before removing a tooltip
    ;
  ...
  mctooltip.show = function(o) {
    var pos        = o.pos
      , content    = o.content
      , gravity    = o.gravity || 's'
      , dist       = typeof o.dist === 'undefined' ? 10 : o.dist
      , parent     = o.parent
      , classes    = o.classes
      ...
      switch (gravity) {
        case 'e':
          ...
          if (tLeft < scrollLeft) // check if left edge is off screen
            left = pos[0] > scrollLeft - dist ?  //TODO: check math here, also flip gravity, consider checking if right is larger than left
                     pos[0] + dist :
        ...
        default: //case 's':
          ...
          if (scrollTop > tTop) { //check if top edge is off screen
            if (scrollTop - tTop < scrollTop + windowSize.height - tTop - height) { //check if bottom has more room than top
              gravity = 'n';
              calcPos();
            } else {
              top  = scrollTop;
  ...
  mctooltip.cleanup = function() {
    var tooltips = document.getElementsByClassName('mc-tooltip')
      , purging  = []
      , removeMe
      , i
      ;
